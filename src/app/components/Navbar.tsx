  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrolled !== isScrolled) {
        ...
        // Trigger a temporary glitch effect when the background appears
        if (scrolled) {
          setIsGlitching(true);
      ...
      <AnimatePresence>
        {isGlitching && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-[#75feed] mix-blend-overlay z-[-1] pointer-events-none glitch-anim"
            style={{ clipPath: 'polygon(0 0, 100% 0, 100% 10%, 0 10%)' }}
          />
        )}
      </AnimatePresence>

      {/* Cyber grid background when scrolled */}
      <div 
